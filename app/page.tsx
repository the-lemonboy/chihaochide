...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/places").then((r) => r.ok ? r.json() : Promise.reject()).then((rows: (Place & { selected?: number })[]) => {
  ...
  return <main className="shell">
    ...
    <section className="hero"><div className="sparkle">✦</div><p className="eyebrow">FOOD MOOD · 01</p><h1><span>吃啥好吃的呢</span></h1><p className="sub">选择困难没关系，菜单替你操心</p></section>
    <section className="picker card"><div className="section-head"><div><h2>想吃什么</h2><p>先圈起来，纠结稍后再说</p></div><span className="count">{selectedPlaces.length} 家</span></div>
